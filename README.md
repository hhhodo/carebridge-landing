...
5. **기술 섹션 카드 교체**: 사용자가 직접 준비한 5개 3D 보안 아이콘 이미지(환자 신원 보안/진료
   기록 잠금/다중 보안 체계/보안 알림/안전한 문서 보관)로 기존 개발 툴 로고(Notion/Figma/AWS 등)
   카드를 전부 교체하고, 각 이미지 의미에 맞는 캡션 텍스트를 새로 작성했습니다
   (`assets/images/tech-icon-*.png`). 작업 중 실수로 사용자가 넣어둔 원본 파일을 임시파일로
   오인해 삭제했다가 사용자가 다시 첨부해 복구한 이력이 있습니다 — 이후로는 assets 폴더의 파일을
   삭제하기 전 반드시 먼저 확인합니다.
